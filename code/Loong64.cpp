...
int Encrypt_64(int Stt[16], int K[16],  int r)
{
    int res[16] ;
...
{
    // Loong-64
    if (C[ 0] == 0x0 and C[ 1] == 0x0 and C[ 2] == 0x0 and C[ 3] == 0x0 and
         C[ 4] == 0x0 and C[ 5] == 0x0 and C[ 6] == 0x0 and C[ 7] == 0x0 and
         C[ 8] == 0xA and C[ 9] == 0x5 and C[10] == 0x7 and C[11] == 0xF and
...
int main()
{
    int P0[16], P1[16], K[16], Diff_Out[16], T[16];
    int correct_pair;
    double Pr = 0;
    double Pr_sum = 0;

    // Differential for Loong-64
    int Diff[16] = {0x0, 0x0, 0x0, 0x0,
                    0x0, 0x0, 0x0, 0x0,
                    0xA, 0x5, 0x7, 0xF,
                    0x0, 0x0, 0x0, 0x0 };

    int R = 16;  // Number of round
    double d = pow(2, 28);      // Number of pairs
    int m_test = 1; //Number of experiments
